...
- 3.0.0: First release of OWLCMS Video, the successor to the separate Cameras and Replays programs.
  ...
  - Replays reads the camera inventory from the shared `cameras.toml` when both modules run on the same machine, and fetches it over HTTP from Cameras' `/api/cameras/config` endpoint when Cameras runs elsewhere.
